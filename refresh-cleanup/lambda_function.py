...
def deleteRDS(instance: dict, rds_client: boto3) -> bool:
    try:
        ...
        # Waiting 2 sec to make sure AWS corrects instance status
        time.sleep(2)
...
def checkInstance(instance: dict):
    ...
    if not instance["DBInstanceStatus"] == "stopped":
        print(
            f"instance {instance_name} is not in status: -stopped- but in status: -{instance['DBInstanceStatus']}-")
        return False
    ...
    if not date_today == irt_formatted:
        print(
            f"{instance_name} deletion date is {irt_formatted} and not today {date_today}")
        return False
    return True
...
def lambda_handler(event, context):
    ...
    if instance_list:
        return f"successfully deleted instance: {instance_list}"
